...
import { USER_TRIP_ACTION, AddUserTripAction, SetUserTripAction, EditUserTripAction,
	 	UpdateUserTripAction } from '../actions/accountTripList.action';
...
export class AccountTripListEffects {
	...
	@Effect() addUserTrip$ = this.actions$
	  ...
	  @Effect() editUserTrip$ = this.actions$
	  .pipe(ofType<EditUserTripAction>(USER_TRIP_ACTION.EDIT_USER_TRIP),
	    switchMap((action) => {
	      this.store.dispatch(new LoadGlobalEventAction());
	      //here http req
	      return of(new UserTripModel(action.payload))
	        .pipe(switchMap(userTrip => [
	          new LoadGlobalEventSuccessAction(),
	          new UpdateUserTripAction(userTrip)
	        ]),
	        catchError(error => {
	          new LoadGlobalEventFailAction(error);
	          return throwError(error);
