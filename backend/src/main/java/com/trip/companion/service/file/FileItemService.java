...
public class FileItemService {
    ...
    public FileItem uploadImage(MultipartFile file) {
        if (isImage(file)) {
            ...
            throw new RuntimeException("File " + file.getOriginalFilename() + " is not an image file, content-type: "
                    + file.getContentType());
        }
    }
    ...
    @Async
    public void removeFileItemAsync(String fileId) {
        removeFileItem(fileId);
    }

    public void removeFileItem(String fileId) {
        fileItemRepository.findById(fileId).ifPresent(this::removeFileItem);
    }
    ...
    private String getFileExtension(MultipartFile file) {
        try {
            ...
            throw new RuntimeException("Failed to detect file mime type");
        }
    }
}
